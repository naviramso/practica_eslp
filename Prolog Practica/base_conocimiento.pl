...
recomendar_peliculas(Generos, ActoresFavoritos, PeliculasRecomendadas) :-
    ...
    , PeliculasRecomendadas = json([ "x" = 1, "y" = 2 ]), json_write(current_output,PeliculasRecomendadas)

    %,PeliculasRecomendadas=json([nombre='John', edad=30]),json_write(current_output,json([ "x": 1, "y": 2 ])).

    busca(_,[]):-false.
    buscar(X,[Y|Z]):- X=Y;buscar(X,Z).
    
    buscar_listas([],_):-false.
    buscar_listas([X|Z],Lista2):-buscar();buscar_listas():- 
